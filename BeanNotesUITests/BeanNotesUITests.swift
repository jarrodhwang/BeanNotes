...
final class BeanNotesUITests: XCTestCase {
    ...
    func testCreateNewNoteOpensEditor() throws {
        ...
        XCTAssertFalse(app.buttons["8 point eraser"].exists)
        app.buttons["Eraser"].tap()
        app.buttons["Eraser"].tap()
        XCTAssertTrue(app.buttons["8 point eraser"].waitForExistence(timeout: 8))
        XCTAssertTrue(app.buttons["16 point eraser"].waitForExistence(timeout: 8))
        XCTAssertTrue(app.buttons["32 point eraser"].waitForExistence(timeout: 8))
        XCTAssertTrue(app.buttons["48 point eraser"].waitForExistence(timeout: 8))
